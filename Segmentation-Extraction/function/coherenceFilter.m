...
%% Run.
    ...
    temp = uint16(water_corMap_filter);
    if write_flag
        imwrite(squeeze(temp(:,:,1)),fullfile(file_path,'water_corMap_filter.tif'));
        for l=2:num_slices
            imwrite(squeeze(temp(:,:,l)),fullfile(file_path,'water_corMap_filter.tif'),'WriteMode','append');
        end
    end
