function [CalTrace,Coherence]=traceExtract(file_path,pre_name,value_name,seg_regions,water_corMap,info_data,start_frame,batch_size,end_frame,write_flag)
...
%   Coherence --- Coherence of segmented regions.
    ...
    CalTrace = zeros(num_regions,T);
    for ff=start_frame:batch_size:end_frame
        Y_r = zeros(num_voxels,min([end_frame,ff+batch_size-1])-ff+1); % load a batch
        for i=ff:min([end_frame,ff+batch_size-1])
            load(fullfile(file_path,[pre_name,num2str(i),input_extend]),value_name);
            Y_r(:,i-ff+1) = reshape(eval(value_name),[num_voxels,1]);
        end
        for k=1:num_regions
    ...
    for t=1:T
        ...
        Coherence = Coherence + (double(eval(value_name))-info_data.F_mean).*temp;
    end
