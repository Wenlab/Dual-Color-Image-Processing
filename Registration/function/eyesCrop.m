...
function eyesCrop(file_path_red,file_path_green,start_frame,step_size,end_frame,num_index,mask_path,thread_num)
...
%% Run crop eyes.
    ...
    spmd
        if start_frame+(spmd_num*spmdIndex-1)*step_size <= end_frame
            cropEye(file_path_red,file_path_green,start_frame+spmd_num*(spmdIndex-1)*step_size,step_size,start_frame+(spmd_num*spmdIndex-1)*step_size,num_index,Mask);
        else
            cropEye(file_path_red,file_path_green,start_frame+spmd_num*(spmdIndex-1)*step_size,step_size,end_frame,num_index,Mask);
        end
    end
    toc;
