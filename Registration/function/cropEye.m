...
function cropEye(file_path_red,file_path_green,start_frame,step_size,end_frame,num_index,Mask)
...
%% Run.
    for i = start_frame:step_size:end_frame
        ...
        % run crop.
        red_crop_image = red_image(1:380,:,:);
        green_crop_image = green_image(1:380,:,:);

        red_crop_image=uint16(double(red_crop_image).*Mask);
        green_crop_image=uint16(double(green_crop_image).*Mask);
